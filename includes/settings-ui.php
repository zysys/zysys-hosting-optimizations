...
function zysyshosting_admin_panel() {
    ...
    <div class="wrap" style="display:block!important">
        <img src="//zysyshosting.cachefly.net/zysys.org/images/retina-zysys-logo.png" style="width:198px;"
             alt="Zysys Logo"/>
        <h2>Zysys Hosting</h2>
        <p>This panel will give you options to control your site in, hopefully useful ways. If you have any suggestions,
            contact your Zysys Representative.</p>
    </div>
    <hr/>
    <form name="zysyshostingprefs" method="post" action="">
        <h3>Automatic Updates</h3>
        <p class="caption">Turning on automatic updates can break your site long term. If you don't regularly update
            your site, it is recommended that you turn these 3 options on.</p>
        <input type="checkbox" id="ZysysCoreUpdater"
               name="ZysysHostingCoreUpdater" <?php checked(get_option('zysyshosting_update_core_automatically'), 'update1'); ?>
               value='1'/><label for="ZysysHostingCoreUpdater">Keep the WordPress Core Updated</input><br/>
            <input type="checkbox" id="ZysysPluginUpdater"
                   name="ZysysHostingPluginUpdater" <?php checked(get_option('zysyshosting_update_plugins_automatically'), 'update1'); ?>
                   value='1'/><label for="ZysysHostingPluginUpdater">Keep WordPress Plugins Updated</input><br/>
                <input type="checkbox" id="ZysysThemeUpdater"
                       name="ZysysHostingThemeUpdater" <?php checked(get_option('zysyshosting_update_themes_automatically'), 'update1'); ?>
                       value='1'/><label for="ZysysHostingThemeUpdater">Keep WordPress Themes Updated</input><br/>
                    <h2>Zycache Settings</h2>
                    <input type="checkbox" id="ZycacheImages" name="ZycacheImages" <?php checked(USE_ZYCACHE_IMG); ?>
                           value='1'/><label for="ZycacheImages">Use Zycache for images (<em>recommended</em>)</input>
                        <br/>
                        <input type="checkbox" id="ZycacheCSS" name="ZycacheCSS" <?php checked(USE_ZYCACHE_CSS); ?>
                               value='1'/><label for="ZycacheCSS">Use Zycache for stylesheets
                            (<em>recommended</em>)</input><br/>
                            <input type="checkbox" id="ZycacheJS" name="ZycacheJS" <?php checked(USE_ZYCACHE_JS); ?>
                                   value='1'/><label for="ZycacheJS">Use Zycache for scripts
                                (<em>recommended</em>)</input><br/>
                                <h3>Speed Optimizations</h3>
                                <input type="checkbox" id="ZysysHostingImageCompression"
                                       name="ZysysHostingImageCompression" <?php checked(get_option('zysyshosting_keep_images_compressed'), 'compress1'); ?>
                                       value='1'/><label for="ZysysHostingImageCompression">Keep Images Compressed
                                    without Loosing Quality</label><br/>
                                <?php if (isset($optset) && $optset) { ?>
                                    <input type="submit" name="ZysysHostingOptions" disabled style="font-style:italic"
                                           class="button-primary" value="Settings Updated."/>
                                <?php } else { ?>
                                    <input type="submit" name="ZysysHostingOptions" class="button-primary"
                                           value="Update Settings"/>
                                <?php } ?>
    </form>
    <hr/>
    <h2>Maintenance</h2>
    <p>Run the maintenance procedures if you've made a very significant level of adjustments, can't wait for the
        regularly scheduled maintenance interval, or something is wrong.</p>
    <form name="zysyshostingmaintenance" method="post" action="">
        <?php if ($maint) { ?>
            <input type="submit" name="ZysysHostingMaintenance" disabled style="font-style:italic"
                   class="button-primary" value="Zysys Hosting Maintenance Procedures Complete."/>
        <?php } else { ?>
            <input type="submit" name="ZysysHostingMaintenance" class="button-primary"
                   value="Run Zysys Hosting Maintenance Procedures"/>
        <?php } ?>
        <hr/>
        <h2>Install Extra Plugins</h2>
        <p>We recommend certain plugins (some of which we've purchased licenses, others are open source). Use the
            buttons below to install any of them.</p>
    </form>
    <p>We have licensed software for you to use! Install the software below and it will install the license key after it
        activates the plugin. If you need to reinstall all license keys, click this button:</p>
    <form name="zysyshostingrekey" id="zysyshostingrekey" method="post" action="">
        <?php if (isset($_POST['rekey'])) { ?>
            <input type="submit" name="rekey" disabled style="font-style:italic" class="button-primary"
                   value="Reinstallation of License Keys Complete."/>
        <?php } else { ?>
    ...
    <form name="zysyshostingplugins" id="zysyshostingplugins" method="post" action="">
        <table>
            <tr>
                <th>Plugin Purpose</th>
                <th>Plugin Name</th>
                <th>Link to Install/Activate</th>
            </tr>
            <?php
            foreach ($thirdPartyPlugins as $plugin) {
                ?>
                <tr style="text-align:center;border:5px solid #000">
                    <td class="padtd"><?php echo $plugin[0]; ?>
                        <hr/>
                    </td>
                    <td class="padtd"><?php echo $plugin[1]; ?>
                        <hr/>
                    </td>
                    <td><a <?php if (!zysyshosting_is_plugin_installed($plugin[1])) {
                            print "id='install-" . $plugin[2] . "' value='INSTALL " . strtoupper($plugin[1]) . "' class='button' onclick='javascript:jQuery(\"#install-plugin\").attr(\"value\", \"" . $plugin[2] . "\");jQuery(\"#zysyshostingplugins\").submit();'>INSTALL " . strtoupper($plugin[1]);
                        } else {
                            if (is_plugin_active($plugin[4])) {
                                print 'disabled="disabled"';
                            }
                            print "id='activate-" . $plugin[2] . "' class='button' onclick='javascript:jQuery(\"#activate-plugin\").attr(\"value\", \"" . $plugin[2] . "\");jQuery(\"#zysyshostingplugins\").submit();''>ACTIVATE " . strtoupper($plugin[1]);
                        }; ?></a>
                        <hr/>
                    </td>
                </tr>
